...
> ↗ 已蒸馏至 `LEARNINGS.md`「用户质疑是发现盲点的最好方式」
...
> ↗ 已蒸馏至 `LEARNINGS.md`「时间锚混淆：用"当前快照"否定"历史推断"」
...
### 2026-06-20（凌晨）: PowerShell 7 管道传递 emoji 数据导致终端挂起

> ↗ 已蒸馏至 `LEARNINGS.md`「调查工具本身可能是卡死嫌疑人（自我指涉陷阱）」

- **类型**：错误（系统性）/ 调查发现
- **背景**：调查上次会话卡死原因时，用到 PowerShell `Get-ChildItem | ForEach-Object { ... }` 管道链，输出中包含 emoji（✅/❌）；终端静默挂起无返回

- **问题**：PowerShell 7 在 Windows GBK 936 代码页下，通过管道传递含 4 字节 Unicode emoji 的多行脚本块输出时，管道流可能因编码不匹配挂起。表现为命令"正在运行"但永无输出

- **触发条件**（[实测]）：
  1. PowerShell 7 + GBK 936 代码页（中文 Windows 默认）
  2. 管道链中包含 `ForEach-Object` 多行脚本块
  3. 输出中包含 4 字节 Unicode emoji（如 ✅❌⚠️🔍）
  4. 简单管道（如 `Get-Content | Select-Object`）不受影响

- **解决方案**：
  1. 优先：拆成多次简单命令（无管道）
  2. 备选：`cmd /c "..."` 绕过 PowerShell 管道
  3. 备选：`node -e "..."` 用 Node 做文件扫描
  4. 极少数情况：`$OutputEncoding = [Console]::OutputEncoding = [Text.UTF8Encoding]::new()`

- **Q0（系统性 vs 偶然）**：系统性。PS 7 + GBK 936 + emoji = 管道编码炸弹。每次 `ForEach-Object` + emoji 输出都会触发

- **Q1（错在哪）**：使用含 emoji 输出的 PS 管道链，终端在 GBK 编码下无法正确序列化/反序列化 4 字节 Unicode

- **Q2（为什么错）**：PS 7 虽然原生 UTF-8，但 GBK 936 系统代码页下 stdout 管道仍受控制台编码影响；emoji（\u{1F4CB} 等 ≥ U+FFFF）在窄编码管道中被截断或阻塞

- **Q3（现有规则为何没阻止）**：规范 10（命令非交互化检查）覆盖了 pager/REPL/长驻服务/确认提示四类，但**未覆盖"管道 + 编码"导致的隐性阻塞**。规范 7 只说了 PS 7 语法，未警示编码陷阱

- **Q4（规则调整）**：规范 10 补充第 5 类陷阱：「**编码管道陷阱**：PS 7 + GBK 936 下，含 emoji 输出的复杂管道链（`| ForEach-Object { ... }`）可能因 4 字节 Unicode 挂起。改写为单步命令、`cmd /c`、或 `node -e`」
  - 同步在规范 7 加一条 PS 7 警告

- **Q5（未来如何更早发现）**：
  1. PowerShell 命令包含 `| ForEach-Object` 或 `| %` 且脚本块内可能有 emoji → 触发预检
  2. 调查时形成"自我指涉陷阱"意识：当调查工具本身可能是嫌疑人时，命令本身可能也卡死

- **关联规则**：规范 10（补充第 5 类）、规范 7（补充 PS 7 emoji/编码警告）
- **来源**：2026-06-20 凌晨调查"上次会话卡死"时实测触发


### 2026-06-19（夜·续）: handoff 测试套件污染真实工作树 — 22:50 active 文件神秘消失事件破案

> ↗ 已蒸馏至 `LEARNINGS.md`「调查假设池必须包含"非业务代码路径"」
